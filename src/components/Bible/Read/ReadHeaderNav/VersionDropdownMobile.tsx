      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            variant="outline"
            size="sm"
            className="border-border bg-card text-foreground hover:bg-muted/50 min-h-8
              min-w-0 gap-1 rounded-md sm:min-h-9 sm:gap-1.5"
            aria-label={t("readVersion")}
          >
            <span className="truncate text-xs sm:text-sm">{t("readVersion")}</span>
            <ChevronDown className="h-3.5 w-3.5 shrink-0 sm:h-4 sm:w-4" />
          </Button>
        </DropdownMenuTrigger>
          ...
          {leftVersion !== null && (
            <button
              ...
              className={cn(
                ...
                shadow-sm sm:rounded-lg sm:px-3 sm:py-1.5 sm:text-sm`,
