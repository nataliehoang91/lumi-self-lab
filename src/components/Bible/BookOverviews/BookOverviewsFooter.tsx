...
function getBookOverviewSegment(
  pathname: string | null
): { lang: string; bookSlug: string | null } | null {
  ...
  const bookSlug =
    overviewIdx >= 0 && parts[overviewIdx + 1] ? parts[overviewIdx + 1] : null;
...
export function BookOverviewsFooter() {
      ...
      >
        ...
        <Link
          ...
          className={cn(
            ...
            hover:opacity-90 sm:ml-auto sm:w-auto sm:justify-center`,
            bodyClass
          )}
        >
  ...
  const currentIndex = BOOK_SLUGS_ORDER.indexOf(
    bookSlug as (typeof BOOK_SLUGS_ORDER)[number]
  );
  const isLastBook = currentIndex === BOOK_SLUGS_ORDER.length - 1;
  const prevHref =
    currentIndex > 0
      ? `/bible/${lang}/book-overviews/${BOOK_SLUGS_ORDER[currentIndex - 1]}`
      : indexHref;
  const nextSlug =
    !isLastBook && currentIndex >= 0 ? BOOK_SLUGS_ORDER[currentIndex + 1] : null;
  const nextHref = nextSlug ? `/bible/${lang}/book-overviews/${nextSlug}` : readHref;
  const nextBookName =
    nextSlug && currentIndex >= 0 && currentIndex + 1 < BOOK_NAMES_EN.length
      ? lang === "vi"
  ...
  const nextLabel = isLastBook
    ...
    : nextBookName
      ? lang === "vi"
        ? `Đọc tổng quan ${nextBookName}`
        : `Read ${nextBookName} overview`
      : null;
  ...
  return (
    ...
    >
      ...
      >
        {nextLabel ? <span className="font-bold">{nextLabel}</span> : null}
        <ArrowRight className="h-3.5 w-3.5 shrink-0" />
      </Link>
    </div>
  );
