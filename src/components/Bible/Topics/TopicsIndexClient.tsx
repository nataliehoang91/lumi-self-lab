    ...
    <div>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className={cn(
            "border-border bg-card text-foreground placeholder:text-muted-foreground/50 w-full rounded-xl border py-2.5 pr-4 pl-9 outline-none transition-shadow focus:ring-2 focus:ring-second/30",
            bodyClass,
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((topic) => (
            <TopicCard key={topic.slug} topic={topic} segment={segment} />
          ))}
        </div>
      ) : (
