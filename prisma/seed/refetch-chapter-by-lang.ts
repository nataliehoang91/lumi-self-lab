...
function parseArgs(): {
  ...
  for (const a of args) {
    if (a.startsWith("--bookId=")) bookId = a.slice("--bookId=".length).trim();
      ...
      lang = a.slice("--lang=".length).trim().toLowerCase() as
        | "vie"
        | "kjv"
        | "niv"
        | "zh";
  ...
  if (!bookId || !Number.isFinite(chapter) || chapter < 1 || !lang) {
    console.error(
      "Usage: npx tsx prisma/seed/refetch-chapter-by-lang.ts --bookId=ID --chapter=N --lang=vie|kjv|niv|zh [--override]"
    );
    process.exit(1);
  }
...
async function main() {
  ...
  if (lang === "vie") {
    ...
    if (!Array.isArray(ch))
      throw new Error(
        `No Vietnamese data for book index ${bookIndex}, chapter ${chapter}`
      );
    versesInChapter = ch;
  } else if (lang === "kjv") {
    ...
    if (!Array.isArray(ch))
      throw new Error(`No KJV data for book index ${bookIndex}, chapter ${chapter}`);
    versesInChapter = ch;
  } else if (lang === "zh") {
    ...
    if (!Array.isArray(ch))
      throw new Error(`No Chinese data for book index ${bookIndex}, chapter ${chapter}`);
    versesInChapter = ch;
  } else if (lang === "niv") {
    const booksRes = await fetch(BOOKS_JSON);
    if (!booksRes.ok)
      throw new Error(`Failed to fetch NIV Books.json: ${booksRes.status}`);
  ...
  for (let v = 0; v < versesInChapter.length; v++) {
    const verseNum = v + 1;
    const text =
      typeof versesInChapter[v] === "string" ? (versesInChapter[v] as string).trim() : "";
    if (!text) continue;
